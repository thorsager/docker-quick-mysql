...
usage() {
  ...
  echo "" 1>&2
  echo "   Arguments:" 1>&2
  echo "     -l                 tail the log of the container withe name defined by -n" 1>&2
  echo "     -k                 kill any running container with name defined by -n" 1>&2
  echo "     -m <mysql-tag>     image tag for the mysql server, supported 5 and 8 (default: ${IMAGE_TAG})" 1>&2
  echo "     -d <databases>     comma seperated list of databases to be created (default: '${DATABASES}')" 1>&2
